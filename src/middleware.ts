...
// Adsterra/HilltopAds rotating shards. Each one needs BOTH the bare domain
// and the wildcard form because CSP wildcards do not match the apex (silent
// bug in CLAUDE.md). Adsterra rotates these every few weeks — when a new
...
// HilltopAds VAST creative CDN shards. These serve the actual MP4 video
// creative for the preroll. NOT in img/script alone — must also be in
// media-src for <video> to load. Discovered via live probe 2026-05-02:
// VAST zone 6969713 was logging 229 impr/day at $0.008 eCPM because
// media-src was missing these hosts → creative MP4 blocked silently.
const HILLTOPADS_VAST_HOSTS = [
  ...ADSTERRA_HOSTS,
...
const HILLTOPADS_VAST_MEDIA = HILLTOPADS_VAST_HOSTS.flatMap((h) => [
  `https://${h}`,
  `https://*.${h}`,
]).join(" ");

// CrakRevenue fraud verification (adsco.re). Without these, CR scores every
// click as "unverified" and scrubs them at their backend → 0 conversions
// regardless of click volume. Discovered 2026-05-02 via CSP-violation probe.
const ADSCORE_HOSTS =
  "https://c.adsco.re https://4.adsco.re https://6.adsco.re https://*.adsco.re";

// Iframe destinations for CrakRevenue popin/cams + PopAds popunder. Without
// frame-src whitelist, iframe-based offers load HTML but the iframe is empty
// (silent CSP block).
const POPUP_FRAME_HOSTS =
  "https://*.crakrevenue.com https://*.mbjms.com https://*.vlmai-1.com https://*.scptp9.com https://*.popads.net https://*.imglnkx.com";
...
export function middleware(request: NextRequest) {
  ...
  // Referrer-Policy: ad networks (HilltopAds, ExoClick) need at least the
  // origin to attribute traffic correctly. `strict-origin-when-cross-origin`
  // strips path; with that, networks downgrade RPM 15-25%. The standard
  // adult-tube policy is `no-referrer-when-downgrade` — full referer over
  // HTTPS, dropped only when going to HTTP (which never happens for us).
