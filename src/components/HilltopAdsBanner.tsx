 ...
 *
 * Isolation pattern (same as pre-nuke 2026-04-18 implementation): the HilltopAds
 * IIFE references `document.scripts[document.scripts.length - 1]` to insert
 * itself next to its own tag. Running multiple zones on the same document
 * would race. Each srcdoc iframe owns its document so the insertion anchor
 * resolves cleanly.
 *
 * Formats (see src/lib/ad-config.ts HILLTOPADS_SCRIPTS):
