#!/usr/bin/env node
// Taxonomy audit — crawls /tag, /character, /series, static pages only.
// Skips /watch (patterns already identified, would fill disk).
// Includes a disk-guard: if / exceeds 75% full, purge ISR watch cache.
...
function diskUsagePct() {
  try {
    const out = execSync("df -P / | tail -1").toString();
    const cols = out.trim().split(/\s+/);
    return parseInt(cols[4].replace("%", ""), 10);
  } catch {
    return 0;
  }
}

async function purgeWatchCache() {
...
async function checkUrl(url, type, retry = 0) {
  ...
  try {
    const res = await fetch(url, {
      ...
      headers: { "User-Agent": "iku-audit/2.0" },
      redirect: "manual",
    });
...
async function main() {
  console.log("Preparing audit tables...");
  await pool.query(`
  ...
  // Disk guard — check every 30s, purge if >75%
  const diskGuard = setInterval(async () => {
    const pct = diskUsagePct();
    if (pct >= 75) {
      console.log(`⚠️  Disk at ${pct}%, purging watch cache...`);
      const ok = await purgeWatchCache();
      const after = diskUsagePct();
      await notify(`🧹 Disk guard: ${pct}% → ${after}% after purge (ok=${ok})`);
    }
  }, 30000);
...
main().catch(async (e) => {
  console.error(e);
  await notify(`❌ Taxonomy audit crashed: ${e.message}`);
  process.exit(1);
});
